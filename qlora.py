...
@dataclass
class JupyterNotebookCompatibilityArguments:
    f: Optional[str] = field(
        default="", metadata={"help": "A dummy argument to fool ipython."}
    )
...
def local_dataset(dataset_name):
    ...
    # NG: test_size=1 is a workaround; we don't want a test set, but need it, so the code later on doesn't fail
    split_dataset = full_dataset.train_test_split(test_size=1)
...
def train(config_file):
    hfparser = transformers.HfArgumentParser(
        (
            JupyterNotebookCompatibilityArguments,
            ModelArguments,
            DataArguments,
            TrainingArguments,
            GenerationArguments,
    ...
    (
        ...
        args_filename=config_file,
    )
    # hfparser = transformers.HfArgumentParser((
    #     ModelArguments, DataArguments, TrainingArguments, GenerationArguments
    # ))
    # model_args, data_args, training_args, generation_args, extra_args = \
    #     hfparser.parse_args_into_dataclasses(return_remaining_strings=True, look_for_args_file=True,
    #                                          args_filename=config_file)
